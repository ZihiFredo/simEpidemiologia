...
#probContacto = 0.1 # Probabilidad de Contacto entre nodos conectados
...
beta_values = np.arange(0.1, 0.61, 0.02)  # De 0.1 a 1.0 con incrementos de 0.05
probContacto_values = np.arange(0.1, 0.61, 0.02)  # De 0.1 a 1.0 con incrementos de 0.05
...
heatmap_data_regular = np.zeros((len(beta_values), len(probContacto_values)))
heatmap_data_strogatz = np.zeros((len(beta_values), len(probContacto_values)))
heatmap_data_barabbasi = np.zeros((len(beta_values), len(probContacto_values)))

# Simulación para cada combinación de parámetros
for i, beta in enumerate(beta_values):
    print(beta)
    for j, probContacto in enumerate(probContacto_values):
...
plt.imshow(heatmap_data_regular, cmap="hot", origin="lower", aspect="auto",
           extent=[0.1, 0.6, 0.1, 0.6], interpolation="bicubic")  # Interpolación para suavizar
